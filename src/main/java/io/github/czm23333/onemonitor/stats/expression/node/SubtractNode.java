...
public class SubtractNode extends BiNode {
    ...
    public SubtractNode(Node leftOp, Node rightOp) {
        this.leftOp = leftOp;
        ...
        switch (lop) {
            ...
            case List<?> l && rop instanceof Double r:
                return l.stream().map(v -> {
                    ...
                    else throw new IllegalExpressionException("Tried subtracting non-double values");
            ...
            case List<?> l && rop instanceof List<?> r:
                if (l.size() != r.size())
                    throw new IllegalExpressionException("Tried subtracting two lists with different length");
                ...
                return result;
            default:
                throw new IllegalExpressionException("Tried subtracting non-double values");
        }
    }
